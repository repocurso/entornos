...
ssh vagrant@kind sh -c 'cat > /shared/repo-config.sh << EOT 
#!/bin/bash
echo \"========== \$HOSTNAME ==========\"
...
EOF
  ...
  grep "auth =" /etc/containerd/config.toml
  echo R:\$REPO_AUTH: O:\$OLD_AUTH:
  sed -i "s/auth = \"\$OLD_AUTH\"/auth = \"\$REPO_AUTH\"/" /etc/containerd/config.toml
fi
systemctl restart docker
systemctl restart containerd
EOT'

ssh vagrant@kind "export REPO_AUTH='$REPO_AUTH'; sh -c 'echo \$REPO_AUTH';for node in kind-control-plane kind-worker kind-worker2; do docker exec -u root -i --privileged -e REPO_AUTH=$REPO_AUTH \$node /bin/bash -c 'sh /shared/repo-config.sh'; done"

echo "**** images repository configured: $(date)"; echo ""
